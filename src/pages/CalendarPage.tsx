...
import { exportYearToPdf } from '../utils/pdfExport';
import { convertGregorianToEthiopian } from '../utils/ethiopianDate';

export function CalendarPage() {
  const { viewMode, currentDate, events } = useCalendarStore();

  const handleExportPdf = useCallback(() => {
    if (viewMode === 'gregorian') {
      exportYearToPdf('gregorian', currentDate.getFullYear(), events);
    } else {
      const eth = convertGregorianToEthiopian(currentDate);
      exportYearToPdf('ethiopian', eth.year, events);
    }
